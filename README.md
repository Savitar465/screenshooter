...
| Ctrl+B               | Reportar bug (abre su ventana)      |
...
| Ejecución         | **El visor manda**: el caso encima, la evidencia elegida con su paso, «Asignar a», «Ampliar», anotar y «Foco» en una barra sobre ella, ‹ › para recorrerlas y la tira horizontal de capturas debajo; a la derecha, un inspector con pestañas —**Paso** (los números de todos los pasos, **un clic para ir a uno**, y la **ficha completa del activo**: acción, datos, esperado y observaciones enteros, por largos que sean), **Pasos** (la lista, con el veredicto corregible en cada uno) y **Bugs**— y al pie Pasa / Falla / Bloq. / N/A en un grupo (teclas P / F / B / S), «Capturar» y el bug. **Modo foco** (F11): la evidencia a toda la ventana y un mando al pie para seguir marcando sin salir (Esc vuelve), **navegación libre entre pasos** —«← Anterior / Siguiente →», Retroceso, Alt+← y Alt+→— que no toca ningún veredicto y reabre una ejecución terminada, «Reportar bug» en cualquier momento (bloqueante si el paso quedó bloqueado), **atajos globales** para avanzar de paso sin volver a la ventana (Ctrl+Alt+P / Ctrl+Alt+F / Ctrl+Alt+A / Ctrl+Alt+D), la clave de los bugs reportados en cada paso, cronómetro por paso y por caso, y los bugs de esta ejecución en su pestaña, por paso. Fallar o bloquear un paso no corta la ejecución; cerrarla a medias la archiva con lo marcado. Sobrevive al cierre de la aplicación |
...
| Bugs              | El libro de bugs del proyecto: lista de lo reportado con su estado en el gestor —**se alarga al deslizar**, no se pinta entera de golpe—, filtros (todos / abiertos / resueltos) y búsqueda por clave, título o caso; «Traer de Jira» recupera del propio gestor los **errores y mejoras** que creó QAflow en el proyecto —también los reportados desde otro equipo— y actualiza sus estados, **por páginas de 50 que van llegando al deslizar**; cada fila dice de qué tipo es; cola offline con reintento; cada bug abre su ficha en una ventana. Reportar abre el parte en **su propia ventana**: prellenado con el paso del que se levanta —el que se tenía delante en la ejecución, o el fallo o bloqueo más cercano; un paso bloqueado lo trae como «Bloqueante»—, con el paso elegible en «Paso» (cada bug pertenece a uno, y es el que recibe el defecto en Zephyr) y campos reales del gestor (tipo, prioridad, asignado, componentes, versión, etiquetas) cargados del proyecto, con las personas buscadas en Jira según se escribe; crea el issue en Jira, GitHub, GitLab o Azure DevOps y sube las capturas, y se esconde sola mientras se captura la pantalla |
