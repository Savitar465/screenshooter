...
| Reportar bug      | Formulario prellenado con el paso del que se levanta el parte —el que se tenía delante en la ejecución, o el fallo o bloqueo más cercano; un paso bloqueado lo trae como «Bloqueante»—, con el paso elegible en «Paso» (cada bug pertenece a uno, y es el que recibe el defecto en Zephyr) y campos reales del gestor (tipo, prioridad, asignado, componentes, versión, etiquetas) cargados del proyecto, con las personas buscadas en Jira según se escribe; crea el issue en Jira, GitHub, GitLab o Azure DevOps y sube las capturas; lista de bugs reportados con su estado y cola offline con reintento |
