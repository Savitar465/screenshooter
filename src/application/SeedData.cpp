...
namespace {
/// Un paso de ejemplo: acción, datos de la prueba (pueden faltar) y resultado esperado.
struct SeedStep { const char* action; const char* data; const char* expected; };

TestCase make(const char* id, const char* title, const char* suite, Priority prio, CaseStatus status,
              RunOutcome outcome, int daysAgo, const char* pre, std::initializer_list<SeedStep> steps) {
    TestCase c;
    ...
    for (const auto& [a, d, e] : steps) c.steps.append(TestStep{QString::fromUtf8(a), QString::fromUtf8(d), QString::fromUtf8(e)});
    return c;
}
} // namespace
    ...
    QList<TestCase> cases = {
        make("TC-101", "Inicio de sesión con credenciales válidas", "Autenticación", Priority::Alta, CaseStatus::Listo, RunOutcome::Passed, 2,
             "Usuario registrado y verificado. Navegador sin sesión activa.", {
                 ...
                 {"Introducir correo y contraseña válidos y pulsar Entrar", "qa.demo@ejemplo.com / Qa2026!", "Redirige al panel principal en menos de 2 s"},
        ...
        make("TC-102", "Bloqueo tras 5 intentos fallidos", "Autenticación", Priority::Alta, CaseStatus::Listo, RunOutcome::Failed, 2,
             "Usuario registrado.", {
                 {"Introducir contraseña incorrecta 5 veces", "qa.demo@ejemplo.com / clave erronea", "Mensaje de cuenta bloqueada temporalmente"},
                 {"Intentar con la contraseña correcta", "qa.demo@ejemplo.com / Qa2026!", "Sigue bloqueado durante 15 min"}}),
        ...
        make("TC-104", "Pago con tarjeta y cupón de descuento aplicado", "Checkout", Priority::Alta, CaseStatus::Listo, RunOutcome::Passed, 1,
             "Carrito con 2 productos. Cupón QA10 activo (10 %). Tarjeta de prueba 4242.", {
                 {"Ir al carrito y pulsar \"Finalizar compra\"", "2 productos · 84,00 €", "Se muestra el resumen con 2 líneas y subtotal correcto"},
                 {"Introducir el cupón y aplicar", "Cupón QA10 (10 %)", "El total baja un 10 % y aparece la línea \"Descuento\""},
                 {"Completar los datos de la tarjeta y pagar", "4242 4242 4242 4242 · 12/30 · CVV 123", "Pantalla de confirmación con número de pedido"},
                 {"Abrir el correo de confirmación", "Buzón de qa.demo@ejemplo.com", "Importe coincide con el total con descuento"}}),
        make("TC-105", "Envío gratuito por encima de 50 €", "Checkout", Priority::Media, CaseStatus::Listo, RunOutcome::Passed, 5,
             "Carrito con importe > 50 €.", {
                 {"Revisar el resumen del pedido", "Carrito de 62,50 €", "Línea de envío muestra 0,00 €"},
                 {"Reducir el carrito por debajo de 50 €", "Quitar un producto · 38,00 €", "Se cobra envío estándar 4,95 €"}}),
        make("TC-106", "Cambiar avatar de perfil", "Perfil", Priority::Baja, CaseStatus::Listo, RunOutcome::None, 0, "Sesión iniciada.", {
                 {"Subir imagen PNG", "avatar-2mb.png", "Vista previa y guardado correcto"},
                 {"Subir imagen demasiado grande", "avatar-12mb.png", "Error claro de tamaño máximo"}}),
