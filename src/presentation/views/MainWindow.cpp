...
MainWindow::MainWindow(AppContext& ctx, QWidget* parent) : QMainWindow(parent), m_ctx(ctx) {
    ...
    m_sidebar = new Sidebar(*ctx.cases, *ctx.plan, *ctx.run);
    h->addWidget(m_sidebar);

    m_stack = new QStackedWidget;
    m_cases = new CasesView(*ctx.cases, *ctx.run);
...
void MainWindow::wireSignals() {
    ...
    connect(m_run, &RunView::finishedAndBack, this, [this]() {
        if (m_ctx.run->isRunning()) showToast(QStringLiteral("Siguiente caso del plan · quedan %1").arg(m_ctx.run->queuedCount() + 1), theme::Green);
        else navigate(Screen::Casos);
    });
